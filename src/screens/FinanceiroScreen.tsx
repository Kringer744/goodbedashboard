...
export function FinanceiroScreen({ data, isLoading }: Props) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      {/* ── Dashboard de Inadimplência & Caixa Vivo (Blueprint) ── */}
      <motion.div
         initial={{ y: 24, opacity: 0 }}
         whileInView={{ y: 0, opacity: 1 }}
         viewport={{ once: true }}
         ...
         className="mb-16"
      >
         <div className="flex items-center gap-4 mb-10 border-l-[6px] border-l-red-500 pl-5">
            <div>
               <h2 className="text-[1.8rem] font-black text-[#1E293B] tracking-tight">Análise de Recebíveis e Inadimplência</h2>
               <p className="text-[13px] text-slate-400 font-semibold mt-1">Comparativo de Caixa Líquido vs. Boletos Atrasados (Simulação de Dashboard V2)</p>
            </div>
         </div>

         <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-gradient-to-br from-[#1E293B] to-[#0F172A] rounded-[2.5rem] p-10 relative overflow-hidden shadow-2xl">
               <div className="absolute top-0 right-0 w-64 h-64 bg-red-500/10 rounded-full blur-3xl -mr-20 -mt-20" />
               <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-6">
                     <AlertCircle size={20} className="text-red-400" />
                     <span className="text-[12px] font-black tracking-widest uppercase text-white/50">Recuperação de Crédito</span>
                  </div>
                  <h3 className="text-white text-[2rem] font-black leading-tight mb-2">
                     Monitoramento de Inadimplência Estrita
                  </h3>
                  <p className="text-slate-400 text-[14px] leading-relaxed max-w-lg mb-8">
                     Visualização da saúde real da rede consumindo <code className="bg-white/10 px-1 font-mono text-[11px] rounded">/api/v1/receivables</code> cruzado com pagamentos negados nos últimos 30 dias.
                  </p>
                  
                  <div className="grid grid-cols-2 gap-4">
                     <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem]">
                        <p className="text-[10px] uppercase font-black tracking-[0.2em] text-white/40 mb-2">Ticket Inadimplente</p>
                        <p className="text-[2rem] font-black text-red-400 tracking-tighter">18.4%</p>
                        <p className="text-[12px] font-semibold text-white/60 mt-2">Média saudável: {'<'} 12%</p>
                     </div>
                     <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem]">
                        <p className="text-[10px] uppercase font-black tracking-[0.2em] text-white/40 mb-2">Perda do Mês (Caixa)</p>
                        <p className="text-[2rem] font-black text-white tracking-tighter leading-none mb-1">
                           <span className="text-red-400">R$</span> 84k
                        </p>
                        <p className="text-[12px] font-semibold text-white/60 mt-1">Impacto negativo na meta</p>
                     </div>
                  </div>
               </div>
            </div>

            <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] flex flex-col justify-between">
               <div>
                  <div className="w-12 h-12 bg-emerald-50 rounded-[1.2rem] flex items-center justify-center mb-6">
                     <DollarSign size={24} className="text-emerald-500" />
                  </div>
                  <h3 className="text-[1.4rem] font-black text-slate-800 leading-tight mb-2">
                     Caixa Liquidado (Em breve)
                  </h3>
                  <p className="text-[13px] text-slate-500 font-medium leading-relaxed">
                     Esta aba substituirá o MRR Projetado pelo dinheiro físico validado em conciliação da Evo, extraindo exatamente as vendas pagas (Sales API).
                  </p>
               </div>
               
               <button disabled className="mt-8 flex items-center justify-center gap-2 w-full py-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-400 font-black text-[11px] tracking-widest uppercase cursor-not-allowed">
                  Módulo Premium V2 <ArrowUpRight size={14} />
               </button>
            </div>
         </div>
      </motion.div>
